...
function updateShowItems(diverror,divok,bsubmit)
{
	...
	$.ajax({
		type: 'POST',
		url: siteurl + 'ajax/admin-general/r:' + Math.round(Math.random()*1000),
		data: 'todo=4&na=' + numactivities + '&nfs=' + numfollowers + '&nfg=' + numfollowing + '&nn=' + numnotifications + '&nim=' + numitemmsg + '&nf=' + numfavorites + '&ns=' + numsearch,
		success: function(resp){
			switch(resp.charAt(0)){
				case '0':
					openandclose(diverror,resp.substring(3),2000);
...
function restorephoto(codep) {
	if (confirm(txtrestore)) {		
		$.ajax({
			type: 'POST',
			url: siteurl + 'ajax/admin-restorephoto/r:' + Math.round(Math.random()*1000),
			data: 'cph=' + codep,
			success: function(resp){
				switch(resp.charAt(0)){
					case '0':
...
function deletephoto(codep) {
	if (confirm(txtdelete)) {	
		$.ajax({
			type: 'POST',
			url: siteurl + 'ajax/admin-deletephoto/r:' + Math.round(Math.random()*1000),
			data: 'cph=' + codep,
			success: function(resp){
				switch(resp.charAt(0)){
					case '0':
